...
#include "Materials/MaterialExpressionSMin.h"
...
UMaterialExpressionSMin::UMaterialExpressionSMin(const FObjectInitializer& Obj):Super(Obj)
{
	struct FConstructorStaics
	{
		FText Name_Math;
		FConstructorStaics() :Name_Math(LOCTEXT("Math", "Math")) {}
	};

	static FConstructorStaics con;
	ConstA = 1.0f;
	ConstB = 1.0f;
	ConstK = 1.0f;
...
#if WITH_EDITOR
int32 UMaterialExpressionSMin::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 arg1 = A.GetTracedInput().Expression ? A.Compile(Compiler) : Compiler->Constant(ConstA);
	int32 arg2 = B.GetTracedInput().Expression ? B.Compile(Compiler) : Compiler->Constant(ConstB);
	int32 arg3 = K.GetTracedInput().Expression ? K.Compile(Compiler) : Compiler->Constant(ConstK);

	return Compiler->SMinShaderToy(arg1, arg2, arg3);
}

void UMaterialExpressionSMin::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(FString("SMin Expression"));
}
...
FName UMaterialExpressionStrataVerticalLayering::GetInputName(int32 InputIndex) const
{
	if (InputIndex == 0)
	{
		return TEXT("Top");
