...
#include "MetaBall/MetaBallShader.h"
...
void FScene::RemoveMetaBallRender()
{
	FScene* Scene = this;
	ENQUEUE_RENDER_COMMAND(FRemoveMetaBallCommand)(
...
FLightSceneChangeSet FScene::UpdateAllLightSceneInfos(FRDGBuilder& GraphBuilder)
{
	...
	for (int32 LightId : ChangeSet.RemovedLightIds)
	{
		...
		FScopeCycleCounter Context(LightSceneInfo->Proxy->GetStatId());
		RemoveLightSceneInfo_RenderThread(LightSceneInfo);	// 移除光源，并且通知Shadow系统
	}
